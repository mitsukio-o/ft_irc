...
| `PASS` | `<password>` | Used to set a "connection password". If the server requires a password, clients must provide the password before attempts to register a connection (before `NICK`/`USER` combination). |
